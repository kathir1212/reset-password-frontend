...
function Bookticketchecklist() {
      ...
      const [data, setData] = useState(null);
        ...
        useEffect(() => {
          const fetchShowtimeData = async () => {
            try {
              const res = await axios.get(`https://reset-password-backend-2.onrender.com/seat/${id}`);
  ...
  const handlePost = async () => {
    try {
      const response = await fetch("https://reset-password-backend-2.onrender.com/booking/ticket", {
        method: "POST",
        headers: {
