...
function Threaterlist() {
  ...
  let threaterlistapi = async () => {
   ...
   await axios.get(`https://reset-password-backend-2.onrender.com/threater/threaterlist`)  
    .then(res => {  
  ...
  const fetchMoviesid = async (query, newPage = 1, reset = false , id) => {
    try {
      const response = await axios.get(`https://reset-password-backend-2.onrender.com/movieuser/movies/${searchQuery}/${id}`);
  ...
  let Showtimeapi = async (id) => {
   ...
   await axios.get(`https://reset-password-backend-2.onrender.com/seat`)  
    .then(res => {  
