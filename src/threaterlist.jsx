...
function Threaterlist() {
  ...
  let threaterlistapi = async () => {
   ...
   await axios.get(`http://localhost:3000/threater/threaterlist`)  
    .then(res => {  
  ...
  const fetchMoviesid = async (query, newPage = 1, reset = false , id) => {
    try {
      const response = await axios.get(`http://localhost:3000/movieuser/movies/${searchQuery}/${id}`);
  ...
  let Showtimeapi = async (id) => {
   ...
   await axios.get(`http://localhost:3000/seat`)  
    .then(res => {  
